...
from Controller import GetMoveMi, SetMoveM
...
class Board(object):
    ...
    def winner(self):
        ...
        # If the game has a winner, it returns the player number (Computer = 1,
        # Human = -1).
...
class Terrain(Canvas):
    ...
    def findBestMove(self, factor):
        ...
        b1 = (self.b.board)
        # BEST Move Param
        bestMove = MTCS(10, o, factor)
        self.b = copy.deepcopy(bestMove.state)
        b2 = (self.b.board)
        print("AIColumn:" + str(FindColumn(b1, b2)))
        print(b2)
        SetMoveM(FindColumn(b1, b2))
        for i in range(6):
            for j in range(7):
