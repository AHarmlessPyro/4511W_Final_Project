...
class Minimax(object):
    ...
    def bestMove(self, depth, state, curr_player,heuristic = 1):
        ...
        for col in range(7):
            ...
            if self.isLegalMove(col, state):
                ...
                legal_moves[col] = -self.search(depth-1, temp, opp_player,heuristic)
    ...
    def search(self, depth, state, curr_player,heuristic):
        ...
        if depth == 0 or len(legal_moves) == 0 or self.gameIsOver(state):
            ...
            return self.value(state, curr_player,heuristic)
        ...
        for child in legal_moves:
            ...
            alpha = max(alpha, -self.search(depth-1, child, opp_player,heuristic))
        return alpha
    ...
    def value(self, state, color,heuristic):
        ...
        if heuristic == 1:
            if opp_fours > 0:
                ...
                return my_fours*100000 #+ my_threes*100 + my_twos*10 - opp_threes*100 - opp_twos*10
        elif heuristic == 2:
            if opp_fours > 0:
                return -100000
            else :
